...
'''This file contains functions including ReadFromFile, Yahoo, YahooMC, GetFromPrice, GetRF
   They are used to prepare necessary data for optimization
   
   Created and Edited by Chaoyi Ye
'''
import pandas as pd
...
'''This function would read bounds provided by users if they are in 3rd and 4th columns
'''
def GetBoundry(df) :
    df.dropna(axis=0,how='any')
    min_bounds = df.iloc[:,2].tolist()
    max_bounds = df.iloc[:,3].tolist()
    weight_bounds = list(zip(min_bounds,max_bounds))
...
'''
def Yahoo(df, startY, endY, startM = 1, endM = 1,benchmark='VOO'):
    df.dropna(axis=0,how='any') #drop all rows that have any NaN values
    Names = df.iloc[:,0] #first column
    tickerList = list(Names) #convert Series to array
    tickerList.insert(0,benchmark)
    ...
    priceA = DataFrame({tic: data['Adj Close']
                        for tic, data in all_data.items()})
    #drop all rows that have any NaN values, i.e. price is not availble
    priceA.dropna(axis=0,how='any',inplace=True)
    benchmark=priceA.iloc[:,0]
    price=priceA.iloc[:,1:]
